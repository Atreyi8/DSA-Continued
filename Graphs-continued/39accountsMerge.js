...
var accountsMerge = function (accounts) {
  let map = new Map();
  let ds = new DisjointSet(accounts.length);
  for (let i = 0; i < accounts.length; i++) {
    let node = i;
    for (let j = 1; j < accounts[i].length; j++) {
      let mail = accounts[i][j];
      if (map.get(mail) === undefined) {
        map.set(mail, i);
      } else {
        ds.unionBySize(map.get(mail), node);
      }
    }
  }

  //creating array of mergemail who have same parent
  let mergedMail = Array(accounts.length)
    .fill()
    .map(() => []);
  for (const [mail, node] of map) {
    //console.log(`${mail}: ${node}`);
    let pu = ds.findParent(node);
    mergedMail[pu].push(mail);
  }

  //console.log("mergedMail",mergedMail)
  //Removing empty arrays and appending name
  let ans = [];
  for (let i = 0; i < accounts.length; i++) {
