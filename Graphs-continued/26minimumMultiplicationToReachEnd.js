...
class Solution {
  minimunSteps(start, end, arr) {
    console.log("here");
    let mod = 100000;
    let distance = Array(mod).fill(Infinity);
    ...
    console.log("q", q);
    while (q.length > 0) {
      let [steps, curr] = q.shift();
      console.log("steps", steps, curr);
      if (curr === end) return steps;
      for (let num of arr) {
        let newCurr = (curr * num) % mod;
        if (distance[newCurr] > steps + 1) {
          distance[newCurr] = steps + 1;
          q.push([steps + 1, newCurr]);
        }
      }
    }
    return -1;
...
console.log(sol.minimunSteps(7, 66175, [3, 4, 65]));
